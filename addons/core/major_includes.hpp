#define MOD_NAME KJW_Imposters
#define MOD_NAME_BEAUTIFIED KJW's Imposters
...
#define COMPONENT_PRESTART COMPONENT_GENERAL##_PreStart
...
#define GET_NUMBER(config,default) (if (isNumber (config)) then {getNumber (config)} else {default}) // Credit to Jenna for these two
