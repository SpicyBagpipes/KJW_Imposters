...
private _cfgVehs = "_side = getNumber (_x >> 'side'); _sim = getText (_x >> 'simulation'); if (((_sim != 'house') && ((_side == 3))) || (901 in (getArray (_x >> 'allowedSlots')))) then {true} else {false};" configClasses (configFile >> "CfgVehicles");
